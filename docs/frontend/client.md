...
Префиксы в `apps/admin/vite.api-prefixes.ts`: `/auth`, `/health`, `/docs`, `/tenants`, `/imports`, `/mock-source`, `/cases`, `/drafts`, `/outbox`, `/replies`, `/tasks`, `/events`, `/crm`, `/dlq`, `/metrics`, `/kill-switch`, `/suppression`.
