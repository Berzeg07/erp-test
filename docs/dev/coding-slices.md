...
**Статус:** ✅ 2026-09-25. `RawLeadRecord` unique `(tenantId, source, externalId)`. JSON `{ leads }` / `{ csv }`, `text/csv`, `GET /mock-source/leads`, `POST /imports/from-mock-source`, список `GET /imports/raw`. Чужой tenant в файле пропускается, не пишется.
...
**Статус:** ✅ 2026-09-25. `POST /crm/sync` upsert: компания (tenant+домен), контакт (tenant+email), сделка (tenant+leadCaseId), задача (tenant+follow_up+leadCaseId). Повтор — те же id. `x-crm-fault: 429|500` → 3 попытки в запросе, `GET /dlq`, CRM пуст. `POST /dlq/:id/reprocess` без заголовка пишет теми же ключами. Списки `GET /crm/{companies,contacts,deals,tasks}`.
