...
| POST | `/crm/sync` | JWT + `X-Tenant-Id` | upsert 4 сущностей mock CRM; сбой `x-crm-fault` |
| GET | `/crm/companies` | JWT + `X-Tenant-Id` | компании mock CRM |
| GET | `/crm/contacts` | JWT + `X-Tenant-Id` | контакты mock CRM |
| GET | `/crm/deals` | JWT + `X-Tenant-Id` | сделки mock CRM |
